 ...
 * @param {String} path Only used for registerPath.
 * @example
 ...
 * });
 */
Injector.prototype.register = function (name, fn, path) {
 ...
 * @param {nameMakerCallback} [nameMaker] A function that creates a name for a module registered by path.
 * @param {NodeModule} [mod] The module to run [require]{@link https://nodejs.org/api/modules.html#modules_module_require_id} on. Defaults to the Injector module, which should typically behave correctly.
 ...
 * @param {Object.<string, string>|string[]} imports Object with keys as injector names and values as module names to import.
 * @param {NodeModule} [mod] The module to run [import]{@link https://nodejs.org/api/modules.html#modules_module_require_id} on. Defaults to the Injector module, which should typically behave correctly.
 * @example
...
Injector.prototype.registerImports = function (imports, mod) {
    ...
    let resolvedModule = mod || module;

    if (Array.isArray(imports)) {
        // Import each import and register with the default name.
        imports.forEach(name => {
            const importedModule = esm(resolvedModule, {})(name);
        ...
        for (var name in imports) {
            if (imports.hasOwnProperty(name)) {
                const importedModule = esm(resolvedModule, {})(imports[name]);
                this.registerValue(name, importedModule.default || importedModule);
            }
        }
    }
...
Injector.prototype.resolve = function () {
    var fn, names, locals, isArray = false, context;
    ...
    if (typeof arguments[0] === "function") {
        ...
        // An array of names was passed in.
        names = arguments[0];
        context = arguments[1];
        isArray = true;
    } else {
        // A single name was passed in.
        names = [arguments[0]];
        context = arguments[1];
        locals = arguments[2];
    }
...
/**
 * Resolve a module with the given name.
 * @param {string} name The name of the module to resolve.
 * @param {Object.<string, *>} [locals] Local variables to inject into the function.
 * @param {string} [context] Optional context to give for error messages. If omitted, path will be used.
 * @example
 * var log = injector.resolveModule("module:get-apps");
 * @returns {*} The result of the executed function.
 */
Injector.prototype.resolveModule = function (name, locals, context) {
    const foundModule = this._graph[name];
    if (!foundModule) {
        throw new Error(`Module not found: ${name}`);
    }

    const { path: p } = this._graph[name];

    // Load the function from the given path.
    var fn = require(p.startsWith('/') ? p : path.join(process.cwd(), p));

    // Resolve the function and return the result.
    return this.resolve(fn, locals, context || name);
};
 ...
 * @private
 */
Injector.prototype._resolve = function (name, previousNames, locals) {
    ...
    if (!node.hasOwnProperty("value")) {
        ...
        var dependencies = node.dependencyNames.map(function (dependencyName) {
            return (locals && locals[dependencyName]) || this._resolve(dependencyName, currentNames);
        }.bind(this));
